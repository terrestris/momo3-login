...
folder are mapped to the application's root namespace, 'MoMo.login'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "MoMo.login"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'MoMo.login' in the same way
as `"MoMo.login/sass/src"`.
